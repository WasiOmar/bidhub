-- =============================================================================
-- db/07_seed.sql
-- BidHub — multi-domain demo data. Proves the platform is genuinely a
-- general-purpose auction marketplace (not a single-category site) and
-- exercises every one of the ten graded techniques with real rows to point
-- at during the demo.
-- Owner: Person 3 (Application)
--
-- Idempotent: TRUNCATEs everything and rebuilds from zero, so re-running is
-- always safe.
--
-- Login credentials (all seeded accounts share the same demo password):
--   password:  password123
--   admin:     admin@bidhub.local
--   sellers:   seller-electronics@bidhub.local, seller-art@bidhub.local,
--              seller-vehicles@bidhub.local, seller-instruments@bidhub.local,
--              seller-books@bidhub.local
--   buyers:    buyer1@bidhub.local .. buyer14@bidhub.local
--
-- Run:  psql "$DATABASE_URL" -f db/07_seed.sql   (after db/01..db/06)
-- =============================================================================
...
-- ===========================================================================
-- 1. Users (~20): 1 admin, 5 sellers (one per domain below), 14 buyers.
-- Every password_hash below is the SAME bcrypt hash (12 rounds) of the
-- literal string 'password123' -- generated once via the server's own
-- bcrypt dependency (`node -e "require('bcrypt').hash(...)"`), not typed by
-- hand, so it is a real, verifiable hash rather than a placeholder string.
-- ===========================================================================
...
-- ===========================================================================
-- 2. Categories: 5 unrelated domains, 4 levels deep each (20 categories
-- total) -- the evidence that BidHub is not a single-category site. Chained
-- INSERT...SELECT so each level's parent_id is looked up from the row just
-- inserted, rather than hand-numbering category_ids.
-- ===========================================================================
...
-- Art & Collectibles -> Paintings -> Oil Paintings -> Landscape Oil Paintings
...
-- Vehicles -> Cars -> Classic Cars -> Muscle Cars
...
-- Books -> Rare Books -> First Editions -> Signed First Editions
...
-- ===========================================================================
-- 3. Items (~60): 12 per leaf category, domain-appropriate JSONB attributes
-- -- a laptop stores ram/cpu/gpu, a painting stores artist/medium/dimensions,
-- a car stores make/model/mileage, and so on. This is what items.attributes
-- being JSONB (rather than a fixed column set) actually buys the platform.
-- ===========================================================================
...
-- --- Art & Collectibles: Landscape Oil Paintings -----------------------------
WITH item_data (title, condition, attrs) AS (
    VALUES
...
-- ===========================================================================
-- 4. Auctions (~40): 8 per domain (items 1-8 of each domain's 12; the last
-- 4 items per domain stay unlisted, as a real marketplace would have)
--   - 3 ACTIVE, staggered end times (one within minutes -- close_expired_
--     auctions() has something to do live in the demo; the other two hours/
--     days out so the site still has live auctions to browse afterward)
--   - 1 SCHEDULED (not open yet)
--   - 3 "already expired" with a reachable reserve (or none) -- picked up
--     and genuinely closed-with-a-sale by step 6's CALL close_expired_
--     auctions() below, via the real trigger + award_winner() path
--   - 1 "already expired" with an unreachably high reserve -- closed the
--     same way, but settles as "no sale"
-- All rows start life as status = 'ACTIVE'; SCHEDULED is the only exception.
-- ===========================================================================
...
-- ===========================================================================
-- 5. Bids (~400): escalating amounts per auction, several auctions with 5+
-- competing bidders. Still-open auctions (future end_time) go through CALL
-- place_bid() for real, so trg_outbid/trg_audit_bid fire naturally and
-- notifications/audit_log populate themselves. Already-expired auctions
-- (past end_time) fall back to direct INSERT -- place_bid() would reject
-- them outright (AU002), and that is exactly the "historical" case the plan
-- calls out for the fallback.
-- ===========================================================================
DO $$
DECLARE
    auc            RECORD;
...
BEGIN
    ...
    FOR auc IN
        ...
        WHERE a.status = 'ACTIVE'   -- SCHEDULED auctions get no bids: they are not open yet
        ORDER BY a.auction_id
    LOOP
        eligible := ARRAY(SELECT unnest(all_buyers) EXCEPT SELECT auc.seller_id);
        use_procedure := auc.end_time > now();
        num_bids := 5 + floor(random() * 16)::INT;   -- 5-20 bids per auction
        ...
        FOR i IN 1..num_bids LOOP
            ...
            IF use_procedure THEN
                -- Genuinely open auction: use the real procedure so
                -- trg_outbid/trg_audit_bid fire exactly as they would for a
                -- live bidder, not a seed script.
                BEGIN
                    CALL place_bid(chosen_bidder, auc.auction_id, bid_amount);
                EXCEPTION WHEN OTHERS THEN
                    NULL; -- extremely unlikely (e.g. a random collision); skip and keep seeding
                END;
            ELSE
                -- Already expired: place_bid() would raise AU002 outright,
                -- so this is the "historical closed auction" fallback the
                -- plan calls for. placed_at is backdated so bids land before
                -- the auction's own end_time, in escalating order.
...
-- ===========================================================================
-- 6. Wrap-up: close everything whose end_time has already passed (the 15
-- "already expired, reachable reserve" + 5 "already expired, unreachable
-- reserve" auctions from step 4) via the real explicit-cursor procedure --
-- exactly what an examiner runs by hand to see technique 06 fire. This is
-- also what turns those 20 auctions' bids into real transactions rows
-- (technique 02) with WON/SOLD/AUCTION_CLOSED notifications, and stamps
-- winning_bid_id via award_winner(). The three still-open ACTIVE auctions
-- per domain are left exactly as they are -- some ending within minutes on
-- purpose, so close_expired_auctions() still has live work to do when it is
-- run again during the demo itself.
-- ===========================================================================
...
-- ---------------------------------------------------------------------------
-- Summary: row counts per table, so a clean run is visibly non-empty.
-- ---------------------------------------------------------------------------
