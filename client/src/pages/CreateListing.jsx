...
// Flattens the recursive-CTE tree (GET /api/categories/tree ->
// get_category_tree(), db/04_queries.sql) into an indented option list --
// the same category picker "driven by the tree" as Browse, just as a
// <select> rather than a clickable tree, since a form needs exactly one
// leaf chosen rather than a filter.
...
export default function CreateListing() {
  ...
  // Dynamic key/value rows -> jsonb attributes. This is the whole point of
  // items.attributes being JSONB rather than a fixed column set: a laptop
  // stores ram/cpu, a painting stores medium/dimensions, and the schema
  // never has to change for either.
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch(() => {
        /* form still usable with a manual category id if this fails */
      });
    return () => {
      cancelled = true;
    };
  ...
  // Mirrors the CHECK constraints on auctions (db/01_schema.sql) so the
  // common mistakes are caught before a round trip -- the server (and the
  // constraints themselves) stay the actual authority; this is a UX nicety,
