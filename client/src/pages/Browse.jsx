import { useEffect, useState, useMemo } from 'react';
import { Link } from 'react-router-dom';
import { api } from '../api/client.js';

function formatMoney(amount) {
  return `$${Number(amount).toFixed(2)}`;
}

// Post-order walk that annotates every node with subtreeCount (its own
// direct item count plus every descendant's) -- the same recursive roll-up
// technique get_category_item_counts() (db/04_queries.sql) demonstrates
// server-side. No route currently exposes that function (P3-02 only wired
// up get_category_tree/get_category_breadcrumb), so this page does the
// roll-up itself in JS from the tree + a flat items list, rather than
// widening the API in a prompt that isn't allowed to touch server/.
function annotateCounts(node, directCounts) {
  const ownCount = directCounts.get(node.category_id) || 0;
  const children = node.children.map((child) => annotateCounts(child, directCounts));
  const subtreeCount = ownCount + children.reduce((sum, c) => sum + c.subtreeCount, 0);
  return { ...node, children, subtreeCount };
}

function collectSubtreeIds(node) {
  return [node.category_id, ...node.children.flatMap(collectSubtreeIds)];
}

function CategoryNode({ node, selectedId, onSelect, depth }) {
  const [expanded, setExpanded] = useState(depth < 1);
  const hasChildren = node.children.length > 0;

  return (
    <li>
      <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
        {hasChildren && (
          <button type="button" onClick={() => setExpanded((e) => !e)} style={{ width: 18 }}>
            {expanded ? '−' : '+'}
          </button>
        )}
        <button
          type="button"
          onClick={() => onSelect(node)}
          style={{ fontWeight: selectedId === node.category_id ? 700 : 400 }}
        >
          {node.name} <span className="count">({node.subtreeCount})</span>
        </button>
      </div>
      {hasChildren && expanded && (
        <ul>
          {node.children.map((child) => (
            <CategoryNode
              key={child.category_id}
              node={child}
              selectedId={selectedId}
              onSelect={onSelect}
              depth={depth + 1}
            />
          ))}
        </ul>
      )}
    </li>
  );
}

export default function Browse() {
  const [tree, setTree] = useState([]);
  const [auctions, setAuctions] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [selected, setSelected] = useState(null);

  useEffect(() => {
    let cancelled = false;

    Promise.all([api.get('/categories/tree'), api.get('/items?limit=100'), api.get('/auctions?status=ACTIVE')])
      .then(([treeData, itemsData, auctionsData]) => {
        if (cancelled) return;

        const directCounts = new Map();
        for (const item of itemsData.items) {
          directCounts.set(item.category_id, (directCounts.get(item.category_id) || 0) + 1);
        }

        setTree(treeData.categories.map((node) => annotateCounts(node, directCounts)));
        setAuctions(auctionsData.auctions);
      })
      .catch((err) => {
        if (!cancelled) setError(err.message);
      })
      .finally(() => {
        if (!cancelled) setLoading(false);
      });

    return () => {
      cancelled = true;
    };
  }, []);

  // Selecting a node filters the auction grid to its whole subtree, not
  // just an exact category match -- the same "browse this category" meaning
  // items.js's GET /api/items?category= already uses server-side.
  const visibleAuctions = useMemo(() => {
    if (!selected) return auctions;
    const ids = new Set(collectSubtreeIds(selected));
    return auctions.filter((a) => ids.has(a.category_id));
  }, [auctions, selected]);

  return (
    <div>
      <h1 className="page-title">Browse categories</h1>
      <div className="two-col">
        <nav className="category-tree card">
          {loading && <div className="empty-state">Loading…</div>}
          {error && <div className="form-error">{error}</div>}
          {!loading && !error && tree.length === 0 && <div className="empty-state">No categories yet.</div>}
          {tree.length > 0 && (
            <>
              <button
                type="button"
                onClick={() => setSelected(null)}
                style={{ fontWeight: !selected ? 700 : 400, marginBottom: 8 }}
              >
                All categories
              </button>
              <ul>
                {tree.map((node) => (
                  <CategoryNode
                    key={node.category_id}
                    node={node}
                    selectedId={selected?.category_id}
                    onSelect={setSelected}
                    depth={0}
                  />
                ))}
              </ul>
            </>
          )}
        </nav>

        <div>
          {!loading && visibleAuctions.length === 0 && (
            <div className="empty-state">No active auctions in this category.</div>
          )}
          {visibleAuctions.length > 0 && (
            <div className="grid">
              {visibleAuctions.map((a) => (
                <Link key={a.auction_id} to={`/auctions/${a.auction_id}`} className="card auction-card">
                  <span className="title">{a.item_title}</span>
                  <span className="price">{formatMoney(a.current_high_bid)}</span>
                  <span className="meta">
                    {a.bid_count} bid{a.bid_count === 1 ? '' : 's'}
                  </span>
                </Link>
              ))}
            </div>
          )}
        </div>
      </div>
    </div>
  );
}
