...
// GET /api/items?category=&seller=&q=&page=&limit=
router.get(
  '/',
  asyncHandler(async (req, res) => {
    ...
    if (category) {
      // The whole subtree under `category`, not just an exact match --
      // get_category_tree($1) (db/04_queries.sql) is the same recursive CTE
      // the category page itself uses, so "browse this category" means the
      // same set of categories everywhere in the API.
...
// POST /api/items -- seller-only. Client-side attribute validation is a UX
// nicety at best; the JSONB column itself has no CHECK on shape (that is the
// whole point of a domain-agnostic attributes bag), so nothing to duplicate.
router.post(
  '/',
  requireAuth,
  requireRole('SELLER'),
